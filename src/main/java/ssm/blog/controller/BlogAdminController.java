...
//            blogIndex.addIndex(blog);
    ...
    public String deleteBlog(@RequestParam("ids") String ids,HttpServletResponse response)throws Exception{
        ...
        for (int i = 0;i < idsStr.length;i++){
            int id = Integer.parseInt(idsStr[i]); //分割多个ID来实现多选
    ...
    public String getById(@RequestParam(value = "id") String id, HttpServletResponse response, HttpServletRequest request)throws Exception{
        Blog blog = blogService.getById(Integer.parseInt(id));
//        String jsonStr = JSONObject.toJSONString(blog);
//        JSONObject result = JSONObject.parseObject(jsonStr);
//        ResponseUtil.write(response,result);
        HttpSession session = request.getSession();
        session.setAttribute("mine",blog);
        return "redirect:/admin/modifyBlog.jsp";
    }
}
